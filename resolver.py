"""
resolver.py — Fribb-first AniList→TMDB ID resolver with multi-source fallback.

Tries sources in order of speed + index coverage:

  Tier 1 — Fribb static lookup (in-memory, O(1), ~39% AniList coverage)
            Fast path — zero network latency.
  Tier 2 — AniBridge cross-provider mapping (1 API call, ~150ms)
            Live API that chases relationship targets to build a full cross-DB
            mapping table. Used when Fribb has no entry for this AniList ID.
  Tier 3 — TMDB /find by TVDB or IMDb ID (from Fribb or AniBridge)
            Catches ~1% more.
  Tier 4 — TMDB /search by name + year (catches brand-new anime that haven't
            propagated to mapping DBs yet)
...
async def resolve_tmdb_id(
    ...
    async with httpx.AsyncClient(timeout=15.0) as client:
        ...
        tvdb_id = _first_tvdb_id(fribb_data, None)  # AniBridge already returned if it had one
        if tvdb_id:
...
def _first_tvdb_id(fribb_data: Optional[dict], anibridge_data: Optional[dict]) -> Optional[str | int]:
